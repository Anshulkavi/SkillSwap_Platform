...
// import { Plus, Filter, Star, Heart } from 'lucide-react';
...
//       } catch (err) {
//         const errorMessage = err.response?.data?.detail || err.message || "Failed to fetch listings.";
...
//     setListings(prevListings => [{ ...newListing, is_liked: false }, ...prevListings]);
//   };
...
//       return <div className="text-center p-10 text-gray-500">Loading listings...</div>;
//     }
//     if (error) {
//       return <div className="text-center p-10 text-red-600 bg-red-50 rounded-lg">{error}</div>;
//     }
//     if (listings.length === 0) {
//       return <div className="text-center p-10 text-gray-500 bg-gray-50 rounded-lg">No skill exchange opportunities found. Be the first to post!</div>;
//     }
...
//             <img src={listing.user.avatar || '/api/placeholder/60/60'} alt={listing.user.name} className="h-14 w-14 rounded-full" />
//             <div>
...
//                 <div className="bg-green-100 text-green-800 px-3 py-2 rounded-lg font-medium text-center">{listing.skill_offered}</div>
//               </div>
//               <div>
//                 <p className="text-sm text-gray-600 mb-1">Looking for</p>
//                 <div className="bg-blue-100 text-blue-800 px-3 py-2 rounded-lg font-medium text-center">{listing.skill_wanted}</div>
//               </div>
//             </div>
//           </div>
//           <div className="flex space-x-2 lg:w-48 justify-end">
//             <button onClick={() => setSelectedListing(listing)} className="flex-1 bg-purple-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-purple-700 transition-colors">View Details</button>
//             <button 
...
//           </div>
//           <button onClick={() => setIsCreateModalOpen(true)} className="mt-4 lg:mt-0 bg-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition-colors flex items-center space-x-2">
...
//               <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
...
//               <input type="text" value={skillOfferedQuery} onChange={(e) => setSkillOfferedQuery(e.target.value)} placeholder="e.g., React Development" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
//             </div>
//             <div>
//               <label className="block text-sm font-medium text-gray-700 mb-2">Skill Wanted</label>
//               <input type="text" value={skillWantedQuery} onChange={(e) => setSkillWantedQuery(e.target.value)} placeholder="e.g., UI/UX Design" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
//             </div>
//           </div>
//         </div>
//         <div className="mb-6"><p className="text-gray-600">Showing {listings.length} skill exchange opportunities</p></div>
...
const SkillExchangePage = () => {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return <div className="text-center p-10 text-red-400 bg-slate-800/50 border border-red-500/30 rounded-xl">{error}</div>;
    }
    if (listings.length === 0) {
      return <div className="text-center p-10 text-slate-400 bg-slate-800/50 rounded-xl">No skill exchange opportunities found. Be the first to post!</div>;
    }
    return listings.map((listing) => (
      <div 
        key={listing.id} 
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center space-y-4 lg:space-y-0 lg:space-x-6">
          <div className="flex items-center space-x-4 lg:w-64">
            <img src={listing.user.avatar || '/api/placeholder/60/60'} alt={listing.user.name} className="h-14 w-14 rounded-full border border-slate-600" />
            <div>
          ...
          <div className="flex-1 lg:flex lg:items-center lg:space-x-6">
            <div className="grid md:grid-cols-2 gap-4 lg:flex-1">
              <div>
                ...
                <div className="bg-green-500/20 text-green-300 px-3 py-2 rounded-lg font-medium text-center border border-green-500/30">{listing.skill_offered}</div>
              </div>
              <div>
                <p className="text-sm text-slate-400 mb-1">Looking for</p>
                <div className="bg-blue-500/20 text-blue-300 px-3 py-2 rounded-lg font-medium text-center border border-blue-500/30">{listing.skill_wanted}</div>
              </div>
            </div>
          </div>
          <div className="flex space-x-2 lg:w-48 justify-end">
