  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
        ...
        <div className="mb-4">
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              onKeyPress={(e) => e.key === "Enter" && onAddLearningSkill()}
              className="flex-1 w-full bg-slate-700 border border-slate-600 rounded-lg text-slate-200 placeholder-slate-400 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
              placeholder="Add a skill you want to learn..."
            />
            <button
              onClick={onAddLearningSkill}
              className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors duration-300 flex items-center justify-center"
            >
              <Plus className="h-4 w-4 mr-1" /> Add
            </button>
        ...
        <div className="flex flex-wrap gap-2">
          <AnimatePresence>
            {editedProfile.skillsToLearn &&
              editedProfile.skillsToLearn.map((skill) => (
                <motion.div
                  key={skill}
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  className="bg-indigo-500/20 text-indigo-300 px-3 py-1.5 rounded-full text-sm font-medium flex items-center gap-2"
                >
                  {skill}
                  <button
                    onClick={() => onRemoveLearningSkill(skill)}
          ...
          >
            ...
            <div
              className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${
                !isHighSchool && "lg:grid-cols-3"
              }`}
            >
              <input
                type="text"
                value={eduData.school || eduData.college || ""}
                onChange={(e) =>
                  onEducationChange(
                    level,
                    isHighSchool ? "school" : "college",
                    e.target.value
                  )
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg"
                placeholder={
                  isHighSchool ? "School Name" : "College/University"
                }
              />
              <input
                type="text"
                value={eduData.board || eduData.degree || ""}
                onChange={(e) =>
                  onEducationChange(
                    level,
                    isHighSchool ? "board" : "degree",
                    e.target.value
                  )
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg"
                placeholder={isHighSchool ? "Board" : "Degree"}
              />
              {!isHighSchool && (
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    onEducationChange(level, "specialization", e.target.value)
                  }
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg"
              ...
              <input
                type="text"
                value={eduData.percentage || eduData.cgpa || ""}
                onChange={(e) =>
                  onEducationChange(
                    level,
                    isHighSchool ? "percentage" : "cgpa",
                    e.target.value
                  )
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg"
                placeholder={isHighSchool ? "Percentage" : "CGPA / %"}
              />
              <input
                type="text"
                value={eduData.year || ""}
                onChange={(e) =>
                  onEducationChange(level, "year", e.target.value)
                }
                className="w-full bg-slate-700 border border-slate-600 rounded-lg"
      ...
      <div className="flex justify-end space-x-4 pt-4">
        <button
          onClick={onCancel}
          className="bg-slate-700 hover:bg-slate-600 text-slate-200 font-medium py-2 px-6 rounded-lg transition-colors"
        >
          Cancel
        </button>
        <button
